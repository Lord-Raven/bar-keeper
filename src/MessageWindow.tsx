        ...
        <div className='important-overflow-visible' style={{position: 'relative', flexGrow: '1', left: '1%', width: '98%', alignContent: 'center', zIndex: 2}}>
            ...
            {chatNode()?.presentPatronIds.map((patronId, index) => {
                    if (stage().patrons[patronId]) {
                        return <PatronImage imgUrl={Math.random() > 0.5 ? stage().patrons[patronId].imageUrls[Emotion.neutral] : stage().patrons[patronId].imageUrls[Emotion.neutral]}
                                            xPosition={getCharacterPosition(index, chatNode()?.presentPatronIds.length ?? 1) - (CHARACTER_HEIGHT * SIZE_RATIO) / 2}
